    ...
    if st.session_state['authentication_status']:
        ...
        with home_tab:
            ...
            if curr_app_grp:
                ...
                #curr_table_name = st.selectbox("Table", get_tables_list(curr_user, cursor, curr_app_grp), index = None, placeholder = "Select Table")
                if curr_app_name:
                    ...
                    if genre == "Bulk Upload":
                        ...
                        if file:
                            ...
                            # curr_user = st.session_state["name"]
                            # conn = sql_connnection('localhost',curr_user, '', 'db')
                            # table_name = st.selectbox("Select Table to load data", get_tables(sql_connnection('localhost',curr_user, '', 'db')))
                        ...
                        # curr_user = st.session_state["name"]
                        # curr_table_name = st.selectbox("Choose Table", get_tables(sql_connnection('localhost',curr_user, '', 'db')))
                        # sql_conn = sql_connnection('localhost', curr_user, '', 'db')
                        # curr_table = show_loaded_table(curr_table_name, sql_conn)
                        # conn = mysql.connector.connect(**db_config)
                        # cursor = conn.cursor()
                        #st.dataframe(curr_table)
                        st.write("Edit Table:")
...
# st.subheader("Upload data into MySQL table")
...
#         table_name = st.selectbox("Select Table to Load Data", tables)

#         # Upload button to load data
#         if st.button("Load Data"):
#             if table_name:
#                 with st.spinner("Loading data..."):
#                     load_data_to_mysql(df, table_name, db_config)
#                 show_loaded_table(table_name, db_config)
#             else:
#                 st.error("Please select a table.")
# elif genre == "Manual Upload":
#     st.write("Manual Upload here")
#     users = get_users(db_config)
#     curr_user = st.selectbox("Select User", users)
#     if st.button("Submit"):
#         curr_table_name = st.selectbox("Choose Table", get_tables(sql_connnection('localhost',curr_user, '', 'db')))
# else:
#     a = 0
